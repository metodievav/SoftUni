...
# 8 - Students Count 
SELECT COUNT(sc.student_id) AS 'student_count', u.name AS 'university_name'
...
BEGIN
	UPDATE students AS s
		...
		JOIN courses AS c ON sc.course_id = c.id
	SET is_graduated = 1
	WHERE YEAR(start_date) = year_started AND s.is_graduated = 0;
END$$
DELIMITER ;
;
